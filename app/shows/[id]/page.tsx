      ...
      <Container>
        ...
        <section className="grid grid-cols-1 md:grid-cols-[320px_1fr] gap-12 items-start">
          ...
          <div>
            ...
            <div className="grid gap-2 text-text-secondary mb-8">
              {show.networks && show.networks.length > 0 && (
                <p><span className="text-text-muted">Networks:</span> {show.networks.join(", ")}</p>
              )}
              {show.genres && show.genres.length > 0 && (
                <p><span className="text-text-muted">Genres:</span> {show.genres.join(", ")}</p>
              )}
              {show.numberOfEpisodes && (
                <p><span className="text-text-muted">Episodes:</span> {show.numberOfEpisodes}</p>
              )}
            </div>
            ...
            {show.description && (
              <div className="mb-10">
                ...
                <p className="text-text-secondary leading-relaxed text-lg">{show.description}</p>
              </div>
            )}

            <Card hoverable={false} className="p-8 mb-10">
              <h2 className="text-2xl font-black mb-1">Rate this show</h2>
              ...
              {session ? (
                <form action={createShowReview} className="space-y-6">
